...
```{r}
alcfit_resid <- predict(alc_fit, alc_train) |>
  bind_cols(alc_train$Walc) |>
  mutate(resid = alc_train$Walc - .pred)
...
(sex | fjob | mjob ) / (studytime | absences | famsize) / (medu | fedu | reason) / (nursery | famrel | freetime) / goout
```
...
Constant Variance: Constant Variance is not met because the vertical spread is not constant throughout; there appears to be a fan shape as you move from left to right, and then the vertical spread decreases again.
