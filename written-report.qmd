...
```{r}
alc <- read_csv("~/project/data/student-por.csv")
```
...
```{r}
#| label: create-recipe
alc_rec1 <- recipe(Walc ~ sex + Pstatus + schoolsup + famsup + Mjob + Fjob + freetime + studytime + absences, data = alc) |>
  step_dummy(all_nominal_predictors()) |>
  step_cut(freetime, studytime, breaks = 1) |>
